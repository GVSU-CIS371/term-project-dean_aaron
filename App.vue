...
export default {
  ...
  data() {
    return {
      ...
      uncategorizedOnly: false,  // New property to track uncategorized filter
      statusFilter: 'all',
      sortBy: 'dueDate'
    };
  },
  ...
  methods: {
    ...
    async fetchCategories() {
      try {
        ...
        if (this.selectedCategory === null && this.uncategorizedOnly === false && this.categories.length > 0) {
    ...
    async saveTask() {
      try {
        ...
        this.taskForm = {
          ...
          dueDate: '',
          priority: 'Medium',
          category: this.selectedCategory || '' // Set to selected category or empty string
        };
    ...
    filterByCategory(categoryId, showAll) {
      if (showAll) {
        // Show all tasks
